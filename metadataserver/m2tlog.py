#!/usr/bin/env python3
"""
m2tlog - Metadata-to-Thread Log Analyzer
...
COOKIE_CAPACITY_PATTERN = re.compile(
    r'reporting\s+cookie\s+jar\s+at\s+capacity\s+with\s+(\d+)\s+cookies',
    re.I
)
GET_COOKIE_PATTERN = re.compile(
    r'GetCookie:\s*Task\s+CONTEXT\s+([0-9A-F]+)\s+getting\s+cookie\s+number\s+(\d+)',
    re.I
)
PUT_COOKIE_PATTERN = re.compile(
    r'PutCookie:\s*Task\s+CONTEXT\s+([0-9A-F]+)\s+returning\s+cookie\s+number\s+(\d+)',
    re.I
)
...
def median(values):
    ...
    if not n:
        return 0
    return sorted_values[n // 2] if n % 2 else (
        sorted_values[n // 2 - 1] + sorted_values[n // 2]
    ) / 2


def percentile(values, percentile_val):
    """Calculate percentile of a list."""
    sorted_values = sorted(values)
    if not sorted_values:
        return 0
    index = min(len(sorted_values) - 1, max(0, (len(sorted_values) * percentile_val + 99) // 100 - 1))
...
def lane_info(logfile_path, cpu_count):
    """
    Detect worker lane configuration from log file.
    
    Returns cookie-based lane count (if found) or synthetic count from --cpu argument.
    """
    capacity = None
    observed_cookies = set()
    evidence_line = None
    
    with open(logfile_path, errors='ignore') as f:
        for line in f:
            match = GET_COOKIE_PATTERN.search(line)
            if match:
                observed_cookies.add(int(match.group(2)))
            
            match = COOKIE_CAPACITY_PATTERN.search(line)
            if match:
    ...
    if capacity is not None:
        if capacity <= RESERVED_COOKIES:
            raise ValueError(
                f'cookie capacity {capacity} is not greater than {RESERVED_COOKIES}'
            )
        return {
            'mode': 'COOKIE',
            'capacity': capacity,
            'reserved': RESERVED_COOKIES,
            'lanes': capacity - RESERVED_COOKIES,
            'observed': max(observed_cookies) + 1 if observed_cookies else None,
            'evidence': evidence_line
        }
    
    if cpu_count is None:
        raise ValueError(
            "cookie capacity not found before SAH Running; rerun with -cpu N"
        )
